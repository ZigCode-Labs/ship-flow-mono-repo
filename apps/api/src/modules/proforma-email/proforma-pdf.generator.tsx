...
const ProformaDocument: React.FC<{ proforma: ProformaData }> = ({ proforma }) => {
      ...
      <Page size="A4" style={styles.page}>
        ...
        <View style={styles.headerRow}>
          ...
          <View style={styles.companyInfo}>
            <Text style={styles.companyName}>{proforma.sellerCompanyName}</Text>
            <Text style={styles.gstinText}>GSTIN: {proforma.sellerGstin || 'N/A'}</Text>
          </View>
        </View>
        ...
        <View style={styles.twoColumn}>
          <View style={styles.columnBox}>
            ...
            <Text style={styles.boxValue}>{proforma.customerName || 'N/A'}</Text>
            <Text style={styles.boxLabel}>Address not provided</Text>
            <Text style={styles.boxLabel}>GSTIN: {proforma.customerGstin || 'N/A'}</Text>
          </View>
          <View style={styles.columnBox}>
            <Text style={styles.boxHeader}>Invoice Details</Text>
            <View style={styles.detailRow}>
              <Text style={styles.detailLabel}>Proforma No:</Text>
              <Text style={styles.detailValue}>{proforma.proformaNumber}</Text>
            </View>
            <View style={styles.detailRow}>
              <Text style={styles.detailLabel}>Date:</Text>
              <Text style={styles.detailValue}>{formatDate(proforma.date)}</Text>
            </View>
            <View style={styles.detailRow}>
              <Text style={styles.detailLabel}>Valid Until:</Text>
              <Text style={styles.detailValue}>{formatDate(proforma.validUntil)}</Text>
            </View>
          </View>
        </View>
        ...
        <View style={styles.table}>
          ...
          {proforma.lineItems.map((item, index) => (
            <View key={index} style={index % 2 === 0 ? styles.tableRow : styles.tableRowAlt}>
              ...
              <Text style={[styles.tableCell, { width: '8%', textAlign: 'right' }]}>{item.qty} PCS</Text>
              ...
              <Text style={[styles.tableCell, { width: '8%', textAlign: 'right' }]}>{item.gstPercent}%</Text>
        ...
        <View style={styles.summaryBox}>
          ...
          <View style={styles.summaryRow}>
            ...
            <Text style={styles.summaryValue}>{formatCurrency(proforma.subtotal)}</Text>
          </View>
          <View style={styles.summaryRow}>
            <Text style={styles.summaryLabel}>Taxable Amount:</Text>
            <Text style={styles.summaryValue}>{formatCurrency(proforma.taxableAmount)}</Text>
          </View>
          <View style={styles.summaryRow}>
            <Text style={styles.summaryLabel}>IGST:</Text>
            <Text style={styles.summaryValue}>{formatCurrency(proforma.igst)}</Text>
          </View>
          <View style={styles.summaryRow}>
            <Text style={styles.summaryLabel}>Total Tax:</Text>
            <Text style={styles.summaryValue}>{formatCurrency(proforma.totalTax)}</Text>
          </View>
          <View style={styles.grandTotalRow}>
            <Text style={styles.grandTotalLabel}>Grand Total:</Text>
            <Text style={styles.grandTotalValue}>{formatCurrency(proforma.grandTotal)}</Text>
          </View>
        </View>
        ...
        <View style={styles.amountInWordsBox}>
          ...
          <Text style={styles.amountInWordsValue}>{numberToWords(proforma.grandTotal)}</Text>
        </View>
        ...
        <View style={styles.bottomSection}>
          <View style={styles.bottomBox}>
            ...
            <View style={styles.bottomRow}>
              ...
              <Text style={styles.bottomValue}>N/A</Text>
            </View>
            <View style={styles.bottomRow}>
          ...
          <View style={styles.bottomBox}>
            ...
            <Text style={styles.boxLabel}>{proforma.notes || proforma.paymentTerms || ''}</Text>
          </View>
        </View>
        ...
        <View style={styles.signatoryBox}>
          ...
          <View style={styles.signatoryBody}>
            ...
            <Text style={styles.signatoryCompany}>For {proforma.sellerCompanyName}</Text>
          </View>
        </View>
...
export async function generateProformaPdf(
  ...
  const buffer = await renderToBuffer(<ProformaDocument proforma={proforma} />);
...
<body>
  ...
  <p>Please find attached Proforma Invoice ${proforma.proformaNumber} for your review and confirmation.</p>
  <p><strong>Invoice Details:</strong></p>
  <ul>
    <li>Proforma Number: ${proforma.proformaNumber}</li>
    <li>Total Amount: ${formatCurrency(proforma.grandTotal)}</li>
  </ul>
  <p>Kindly review and confirm your acceptance to proceed with the order.</p>
  <div class="signature">
    <p>Best regards,</p>
    <p>${proforma.sellerCompanyName}</p>
  </div>
</body>
</html>
  `.trim();
