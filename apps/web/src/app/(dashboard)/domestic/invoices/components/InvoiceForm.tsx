...
import { useBuyerSelector } from '@/hooks/useBuyerSelector';
import { CustomerSelectorDropdown } from '@/components/shared/CustomerSelectorDropdown';
...
function getInitialFormData(
  ...
  if (isNew && initialInvoiceNumber) {
    initialState.invoice_number = initialInvoiceNumber;
  }
...
export function InvoiceForm({
  ...
  const [formData, setFormData] = useState<Record<string, unknown>>(() =>
    getInitialFormData(invoice, isNew, initialExchangeRate, initialInvoiceNumber),
  );
  ...
  const { customerOptions, isLoading: isLoadingBuyers, error: buyersError } = useBuyerSelector();
  ...
  useEffect(() => {
    if (isNew && initialInvoiceNumber) {
      setFormData((prev) => ({ ...prev, invoice_number: initialInvoiceNumber }));
    }
  }, [isNew, initialInvoiceNumber]);
  ...
  const validateForm = (): boolean => {
    ...
    const validLineItems = lineItems.filter((item) => item.itemCode.trim());
    if (validLineItems.length === 0) {
      newErrors['lineItems'] = 'At least one line item with an item code is required';
    } else {
      const missingDesc = validLineItems.some((item) => !item.description.trim());
      if (missingDesc) {
    ...
    <div className="h-full flex flex-col bg-gray-50 overflow-hidden">
      ...
      <div ref={formBodyRef} className="flex-1 overflow-y-auto p-4 min-h-0">
        <div className="mx-auto max-w-5xl space-y-4">
          ...
          />
          {errors['lineItems'] && (
            <p className="text-sm text-red-500 mt-1">{errors['lineItems']}</p>
          )}
