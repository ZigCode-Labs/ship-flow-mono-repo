...
interface SendProformaEmailDialogProps {
  open: boolean;
...
export function SendProformaEmailDialog({
  open,
  onOpenChange,
  proforma,
  onSent,
}: SendProformaEmailDialogProps) {
  ...
  const onSubmit = async (formData: SendEmailFormData) => {
    ...
    setIsSending(true);
    try {
      await api.post('/proforma-email/send', {
        ...
        attachPdf,
        pdfFileName: `${proforma.proformaNumber}.pdf`,
      });

      toast.success('Email sent successfully', {
        description: `Proforma invoice sent to ${recipients.map((r) => r.name).join(', ')}`,
      });
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <DialogTitle className="text-base font-semibold text-slate-800">
                  Send Domestic Proforma Invoice via Email
                </DialogTitle>
              </div>
            </div>
            <DialogDescription className="text-sm text-slate-500 mt-1">
              Document {proforma.proformaNumber} will be attached as PDF
            </DialogDescription>
          </DialogHeader>
                      ...
                      >
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="bg-purple-100 text-purple-700 px-1.5 py-0.5 rounded text-[10px] font-medium shrink-0">
                              {result.type}
                            </span>
                          </div>
              ...
              <div className="flex-1">
                <label htmlFor="attachPdf" className="flex items-center gap-2 cursor-pointer">
                  ...
                  <span className="text-sm font-medium text-slate-800">
                    Attach PDF: {proforma.proformaNumber}.pdf
                  </span>
                </label>
                <p className="text-xs text-slate-500 mt-0.5 ml-6">
                  Domestic Proforma Invoice will be attached to the email
                </p>
              </div>
            </div>
              ...
              <div className="flex items-start gap-2 p-3 bg-green-50 border border-green-100 rounded-lg">
                ...
                <div>
                  ...
                  <p className="text-xs text-green-600">
                    This is exactly what {recipients[0]?.name} will receive. No placeholders - all real data!
                  </p>
                </div>
              </div>
            )}
