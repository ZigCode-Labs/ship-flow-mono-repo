...
interface SendCreditNoteEmailDialogProps {
  open: boolean;
...
export function SendCreditNoteEmailDialog({
  open,
  onOpenChange,
  creditNote,
  onSent,
}: SendCreditNoteEmailDialogProps) {
  ...
  }, [open, creditNote.creditNoteNumber, reset]);
  ...
  const onSubmit = async (formData: SendEmailFormData) => {
    ...
    setIsSending(true);
    try {
      await api.post('/credit-note-email/send', {
        creditNote: {
        ...
        attachPdf,
        pdfFileName: `${creditNote.creditNoteNumber}.pdf`,
      });

      toast.success('Email sent successfully', {
        description: `Credit note sent to ${recipients.map((r) => r.name).join(', ')}`,
      });
        ...
        <DialogContent className="sm:max-w-[540px] p-0 gap-0 overflow-hidden">
          <DialogHeader className="px-6 pt-6 pb-2">
            ...
            <DialogDescription className="text-sm text-slate-500 mt-1">
              Document {creditNote.creditNoteNumber} will be attached as PDF
            </DialogDescription>
          </DialogHeader>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="px-6 pb-6 space-y-5">
                      ...
                      >
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="bg-amber-100 text-amber-700 px-1.5 py-0.5 rounded text-[10px] font-medium shrink-0">
                              {result.type}
                            </span>
                          </div>
            ...
            <div className="flex items-start gap-3 p-3 bg-amber-50/60 border border-amber-100 rounded-lg">
              <input
                type="checkbox"
                id="attachPdfCreditNote"
                checked={attachPdf}
                onChange={(e) => setAttachPdf(e.target.checked)}
                className="mt-0.5 h-4 w-4 rounded border-slate-300 text-blue-600 focus:ring-blue-500"
              />
              <div className="flex-1">
                <label htmlFor="attachPdfCreditNote" className="flex items-center gap-2 cursor-pointer">
                  <FileText className="h-4 w-4 text-amber-500" />
                  <span className="text-sm font-medium text-slate-800">
                    Attach PDF: {creditNote.creditNoteNumber}.pdf
                  </span>
                </label>
                <p className="text-xs text-slate-500 mt-0.5 ml-6">
                  Credit Note will be attached to the email
                </p>
              </div>
            </div>
