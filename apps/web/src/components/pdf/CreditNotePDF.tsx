...
const s = StyleSheet.create({
  ...
  cAmt:  { width: '12%', textAlign: 'right' as const },
...
export function CreditNotePDF({ creditNote }: { creditNote: CreditNote }) {
  const total = creditNote.grandTotal || creditNote.amount || 0;
  const items = creditNote.lineItems?.length
      ...
      <Page size="A4" style={s.page}>
        ...
        <View style={s.header}>
          <View>
            ...
            <Text style={s.gstin}>GSTIN: N/A</Text>
          </View>
        ...
        <View style={s.titleBar}>
          <Text style={s.titleText}>CREDIT NOTE</Text>
        </View>

        {/* ── Bill To + Credit Note Details ── */}
        <View style={s.twoCol}>
          <View style={s.boxLeft}>
            <Text style={s.sectionTitle}>BILL TO</Text>
            <Text style={s.boldName}>{creditNote.customerName || 'N/A'}</Text>
            <Text style={s.sub}>GSTIN: {creditNote.customerGstin || 'N/A'}</Text>
            <Text style={s.sub}>Place of Supply: {creditNote.placeOfSupply || 'N/A'}</Text>
          </View>
          <View style={s.box}>
            <Text style={s.sectionTitle}>CREDIT NOTE DETAILS</Text>
            <View style={s.fieldRow}>
              <Text style={s.fieldLbl}>Credit Note No:</Text>
              <Text style={s.fieldVal}>{creditNote.creditNoteNumber || 'N/A'}</Text>
            </View>
            <View style={s.fieldRow}>
              <Text style={s.fieldLbl}>Date:</Text>
              <Text style={s.fieldLbl}>{fmtDate(creditNote.creditNoteDate)}</Text>
            </View>
            <View style={s.fieldRow}>
              <Text style={s.fieldLbl}>Original Invoice No:</Text>
              <Text style={s.fieldVal}>{creditNote.invoiceNumber || 'N/A'}</Text>
            </View>
            <View style={s.fieldRow}>
              <Text style={s.fieldLbl}>Invoice Date:</Text>
              <Text style={s.fieldLbl}>{fmtDate(creditNote.invoiceDate)}</Text>
            </View>
            {creditNote.reason ? (
              <View style={s.fieldRow}>
                <Text style={s.fieldLbl}>Reason:</Text>
                <Text style={s.fieldVal}>{creditNote.reason}</Text>
              </View>
            ) : null}
          </View>
        </View>
        ...
        <View style={s.sumOuter}>
          <View style={s.sumBox}>
            ...
            <View style={s.sumRow}>
              ...
              <Text style={s.sumVal}>{fmt(creditNote.subtotal || 0)}</Text>
            </View>
            <View style={s.sumRow}>
              <Text style={s.sumLbl}>Taxable Amount:</Text>
              <Text style={s.sumVal}>{fmt(creditNote.taxableAmount || 0)}</Text>
            </View>
            <View style={s.sumDiv} />
            <View style={s.sumRow}>
              <Text style={s.sumLbl}>Total Tax:</Text>
              <Text style={s.sumVal}>{fmt(creditNote.totalTax || 0)}</Text>
            </View>
            <View style={s.sumDiv} />
            <View style={s.sumRow}>
        ...
        <View style={s.sigOuter}>
          <View style={s.sigBox}>
            ...
            <Text style={s.sigFor}>For Company Name</Text>
          </View>
        </View>

        {/* ── Footer ── */}
        <View style={s.footer}>
          <Text style={s.footerTxt}>This is a computer-generated credit note.</Text>
