...
export function ProformaInvoiceViewer({
  ...
  const handleDelete = async () => {
    if (window.confirm('Are you sure you want to delete this proforma?')) {
      setIsDeleting(true);
      try {
        const res = await fetch(`http://localhost:4000/domestic-proformas/${proforma.id}`, {
          method: 'DELETE',
        });
  ...
  const handleEmail = () => {
    toast.success('Email Sent', {
      description: `Proforma invoice successfully emailed to ${proforma.customerName || 'customer'}.`,
    });
  };
