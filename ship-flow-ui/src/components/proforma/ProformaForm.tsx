...
export function ProformaForm({
  ...
  const totals = useMemo(() => {
    const subtotal = lineItems?.reduce((sum, item) => sum + getAmount(item), 0) || 0;
    const totalGST = lineItems?.reduce((sum, item) => sum + getItemGSTAmount(item), 0) || 0;
  ...
  const handleSaveProforma = async () => {
    ...
    try {
      ...
      const url = initialData?.id
        ? `http://localhost:4000/domestic-proformas/${initialData.id}`
        : 'http://localhost:4000/domestic-proformas';
      ...
      if (!res.ok) {
        ...
        let errorMessage = 'Failed to save proforma to backend';
